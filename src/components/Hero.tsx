...
export default function Hero() {
  return (
    ...
    >
      {/* Subtle warm glow — morning light through ancient trees */}
      <div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] rounded-full opacity-40 blur-3xl pointer-events-none"
        style={{ background: "radial-gradient(ellipse, #C9922A 0%, transparent 70%)" }}
      />
      <div
        className="absolute top-1/4 right-0 w-[360px] h-[360px] rounded-full opacity-20 blur-3xl pointer-events-none"
        style={{ background: "radial-gradient(ellipse, #C4742A 0%, transparent 70%)" }}
      />
      <div
        className="absolute top-1/3 left-0 w-[320px] h-[320px] rounded-full opacity-15 blur-3xl pointer-events-none"
        style={{ background: "radial-gradient(ellipse, #B5511F 0%, transparent 70%)" }}
      />

      {/* Temple silhouette — ink on parchment */}
      <TempleSceneSVG className="opacity-90" />
      ...
      <div className="relative z-10 text-center px-4 max-w-5xl mx-auto pt-20 pb-32">
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.p
            variants={itemVariants}
            className="text-ink/70 text-xl md:text-2xl font-serif italic mb-4"
          >
          ...
          <motion.p
            variants={itemVariants}
            className="text-ink/55 text-base md:text-lg max-w-2xl mx-auto mb-10 leading-relaxed"
          >
